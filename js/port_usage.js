...
var PortUsage = {
    ...
    update: function() {
        if (serial.bitrate) {
            ...
            // update UI
            $('span.port_usage_down').text(chrome.i18n.getMessage('statusbarUsageDownload', [port_usage_down]));
            $('span.port_usage_up').text(chrome.i18n.getMessage('statusbarUsageUpload', [port_usage_up]));
        } else {
            $('span.port_usage_down').text(chrome.i18n.getMessage('statusbarUsageDownload', [0]));
            $('span.port_usage_up').text(chrome.i18n.getMessage('statusbarUsageUpload', [0]));
        }
    },
    reset: function() {
